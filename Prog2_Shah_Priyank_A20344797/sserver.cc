...
int main(int argc, char* argv[])
{
  ...
  while(1) {
      ...
      bytes = SSL_read(ssl, buf, sizeof(buf)); // retrive client message
      if ( bytes > 0 )
        {
          
	printf("\n\n Client msg received at server side is: \"%s\"\n", buf); //		displaying client message

     	int num = atoi(buf);

        num=num+1;     				//incrementing client msg
	sprintf(reply,"%d",num);

        SSL_write(ssl,reply,strlen(reply)); 	//sending back to client
        }
